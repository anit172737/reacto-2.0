...
import { addTypeQtn, setOpenForm, setSelecetd, editTypeQtn } from "./store";
...
const AddForm = ({ search, pageNumber, pageSize }) => {
  const { selected } = useSelector((state) => state.typeMaster);
  ...
  const onSubmit = async (data) => {
    ...
    if (
      ...
      if (selected) {
        ...
        response = await dispatch(editTypeQtn(modify));
      } else {
        ...
        response = await dispatch(addTypeQtn(payload));
      }
    } else {
      setValue("answer", "");
    }
